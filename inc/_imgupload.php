...
class imgUploder
{
	...
	function __construct($Ufile)
	{
		...
		$this->file_name=htmlentities($Ufile['name'], ENT_QUOTES, "UTF-8");
		$this->file_sname="";
	...
	function chk_FileSize()
	{
		if ($this->file_max < $this->file_size){
			$this->user_msg=$this->user_msg."上傳的檔案大小超過限制".($this->file_max/1024/1024)."M,未成功儲存<br>";
		}
	...
	function chk_Copy()
	{
		if (!move_uploaded_file($this->file,iconv("UTF-8", "big5", $this->file_dir.$this->file_name))) {
			...
			$this->user_msg=$this->user_msg."檔案移動失敗!<br>請上傳".($this->file_max/1024/1024)."MB以下的檔案 ";
		}
	}
	...
	function createImg($target,$target_path,$target_image){
		...
		if (strtolower($this->fileex)=="gif"){
			...
			if (!imagejpeg($target,$targetfile,80)){
				$this->user_msg=$this->user_msg."縮圖建立失敗!<br>";
			}
		}
	}
