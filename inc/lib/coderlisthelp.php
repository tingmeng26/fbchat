...
class coderListHelp{
	...
	*/
	...
	private function drawMainDiv(){
		echo '<div id='.$this->id.' '.($this->ajaxSrc!="" ? ' ajaxsrc="'.$this->ajaxSrc.'" ' : '').($this->delSrc!="" ? ' delsrc="'.$this->delSrc.'" ' : '').($this->editLink!="" ? ' editlink="'.$this->editLink.'" ' : '').($this->orderSrc!="" ? ' ordersrc="'.$this->orderSrc.'" ' : '').($this->excelLink!="" ? ' excelLink="'.$this->excelLink.'" ' : '').($this->addexcelLink!="" ? ' addexcelLink="'.$this->addexcelLink.'" ' : '').($this->csvImportLink!="" ? ' csvImportLink="'.$this->csvImportLink.'" ' : '').'>';
	}
	private function drawButton(){
		...
		echo ($this->csvImportLink!="") ? '<span class="btn btn-circle show-tooltip" id="csvImportfile_btn" title="從csv檔匯入'.$this->name.'" id="csvImportBtn"><i class="icon-inbox"></i></span><label for="csvImportfile" id="csvImportfile_label"></label><input type="file" id="csvImportfile" style="display:none"/>' : '';
		echo '</div>';
		echo '<div class="btn-group">
